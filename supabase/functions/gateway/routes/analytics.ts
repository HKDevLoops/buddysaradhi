...
export const handleAnalytics: RouteHandler = async (_req, db, tenantId, path, method, url) => {
  ...
  if (path === "/api/v1/analytics/dashboard" && method === "GET") {
    ...
    const periodStart = sp.get("periodStartIso") ?? new Date(new Date().setDate(1)).toISOString().slice(0, 10);
    const now = new Date();
    ...
    const [active, payments, invs] = await Promise.all([
      allRows(db, "SELECT balance_paise FROM students WHERE tenant_id = ? AND status = 'active' AND archived_at IS NULL", [tenantId]),
      allRows(db, "SELECT credit_paise FROM ledger_entries WHERE tenant_id = ? AND type = 'PAYMENT_RECEIVED' AND occurred_on >= ?", [tenantId, periodStart]),
      allRows(db, "SELECT total, status, due_date FROM invoices WHERE tenant_id = ? AND due_date >= ?", [tenantId, periodStart]),
    ]);
    ...
    const dueForMonth = invs.filter((i) => i.status !== "paid").reduce((a, i) => a + Number(i.total), 0);
    ...
    const activity = [
      ...
      ...ledger.map((e) => ({
        id: e.id,
        event_type: e.type === "PAYMENT_RECEIVED" ? "PAYMENT" : e.type === "FEE_CHARGED" ? "INVOICE" : "OTHER",
