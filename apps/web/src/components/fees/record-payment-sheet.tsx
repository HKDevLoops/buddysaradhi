...
export function RecordPaymentSheet({ studentId, studentName }: RecordPaymentSheetProps) {
  ...
  const mutation = useMutation({
    mutationFn: (amountMinor: number) => 
      recordPaymentAction(studentId!, amountMinor, description, dateIso),
    onMutate: async (amountMinor) => {
      ...
      // Optimistic Ledger Entry
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      queryClient.setQueryData(['ledger', studentId], (old: { data: any[] } | undefined) => {
        if (!old?.data) return old;
        const newEntry = {
          id: 'temp-' + Date.now(),
          type: 'PAYMENT_RECEIVED',
          debit: 0,
          credit: amountMinor / 100,
          occurred_on: dateIso,
          receipt_no: 'PENDING',
          description,
          isVoid: false,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // The server action then re-validates with Zod.
      ...
      <div className="relative w-full max-w-md h-full glass-strong border-l border-[var(--border-default)] shadow-2xl flex flex-col transform transition-transform duration-300">
        ...
        <div className="p-6 border-t border-[var(--border-default)] bg-[var(--bg-surface-raised)]/30">
          <button 
            ...
            className="w-full neumo-raised py-3 rounded-xl text-sm font-bold text-[var(--text-on-accent)] bg-gradient-to-r from-[var(--accent-emerald)] to-[var(--accent-cyan)] shadow-[0_0_15px_rgba(0,255,157,0.3)] hover:brightness-110 transition-all disabled:opacity-50 disabled:shadow-none"
          >
