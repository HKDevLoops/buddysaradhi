...
│   ├── gateway/        # API Gateway — Supabase Edge Functions (Deno) — canonical backend for all platforms
...
**From now on, the canonical name for the API-Gateway is `@apps/gateway`.** All references to the gateway — in code, docs, PRs, and conversations — must use this canonical form. The gateway lives at `apps/gateway/` and is the single backend entry point for all Buddysaradhi platforms (web, mobile, desktop).

| Attribute | Value |
|-----------|-------|
| **Canonical name** | `@apps/gateway` |
| **Path** | `apps/gateway/` |
| **Runtime** | Deno 2 (Supabase Edge Functions) |
| **Endpoints** | REST (`/api/v1/*`), GraphQL (`/graphql`), Provisioning (`/api/v1/provision`) |
| **DB** | Per-tenant Turso/libSQL (single-tenant SQLite) |
| **Spec** | `17_API_Gateway_System.md`, `deployment/06_Edge_Function_Hosting.md` |

**Why this matters:** The gateway is the backbone of the entire project. Every platform calls it. Every mutation flows through it. Every security boundary is enforced here. When you say "gateway," you mean `@apps/gateway`. No other name, no abbreviations, no informal shortcuts.

### 3.3 Per-App Stack Snapshots (updated)

> This section was 3.2 before adding the `@apps/gateway` reference in §3.2.
...
- **TypeScript 7+ required.** Never reintroduce `typescript-eslint` — it is broken on TS 7 (throws `Cjs undefined error` from `typescript-estree`). ESLint configs use `@eslint/js` recommended + `languageOptions.globals` instead. Do not write legacy CJS patterns (`require()`, `module.exports`).
- **Lint enforcement is zero-warning.** CI lints with `--max-warnings 0`. The pre-commit hook extends lint-staged with a repo typecheck and must pass without `--no-verify`.
...
1. **Recall memory before research.** Before any grep, glob, or codebase search, call `kilo_memory_recall` (mode=search or mode=typed) to check if the answer is already stored. This avoids re-discovering facts that prior sessions already documented — project conventions, prior decisions, environment paths, known bugs.
2. Read `worklog.md` to learn prior context.
3. Read the relevant spec section per §4.
4. Make the smallest correct change.
5. Run `bun run lint`. Fix all errors.
6. (If web) verify in Agent Browser: render + primary interaction + sticky footer.
7. **Update memory after coding work.** After any meaningful code change, update the relevant memory files:
   - `worklog.md` — append a `---`-delimited entry with Task ID, Agent, Task, Work Log, Stage Summary.
   - `kilo_memory_save` — persist durable project facts, decisions, corrections, or new environment paths discovered during the session.
