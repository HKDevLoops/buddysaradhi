...
Both **TestSprite MCP** and **TestSprite CLI** must be used in tandem:

- **TestSprite MCP Server (`@testsprite/testsprite-mcp`)**: Primary tool for **local development and interactive AI testing**. It tunnels your local development server (e.g. `localhost:3000`) so tests can be run and debugged directly in real time.
- **TestSprite CLI (`@testsprite/testsprite-cli`)**: Primary tool for **deployed URLs, staging/preview environments, batch runs, and CI/CD pipelines**.

### Background Task Timeout Requirement

All background operations (including `testsprite test run`, `test create --run`, and background test runners) **MUST specify an explicit timeout** (e.g., `--timeout 600` or process timeout bounds). Never launch a background test task without a bounded timeout.
