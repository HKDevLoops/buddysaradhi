...
- TypeScript 7+ is required. Never reintroduce `typescript-eslint` (broken on TS 7 — Cjs undefined error); ESLint uses `@eslint/js` recommended + globals.
- Lint enforcement is zero-warning: `pnpm run lint` passes `--max-warnings 0` (root script).
- Pre-commit: extends lint-staged with repo typecheck; must pass without `--no-verify`.
